...
class TestGetAlertasView:
    ...
    # TC-C03 — Status 200 no endpoint GET /api/compras/alertas/
    def test_retorna_200_get(self, client):
        resp = client.get("/api/compras/alertas/")
        assert resp.status_code == 200

    # TC-C03 — Contrato: body com chaves criticos e atencao
    ...
    # TC-C03 — Contrato: criticos e atencao são listas
    ...
    # TC-C03 — Contrato: resposta em application/json
    def test_content_type_json(self, client):
        resp = client.get("/api/compras/alertas/")
        assert "application/json" in resp["Content-Type"]

    # Complementar ao TC-C03: método POST não permitido (405)
    def test_nao_aceita_post(self, client):
        resp = client.post("/api/compras/alertas/")
        assert resp.status_code == 405

    # TC-C03 — Cenário: Limiares customizados (critico_max e atencao_max aceitos)
    ...
    # TC-C03 — Cenário: Parâmetros não numéricos (400 + mensagem da especificação)
