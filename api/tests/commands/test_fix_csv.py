...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "..", "..", "management", "commands")
)
...
class TestCriarPastaSaida:
    ...
    def test_criar_pasta_quando_nao_existe(
        self, mock_print, mock_makedirs, mock_exists
    ):
        criar_pasta_saida()
...
class TestSalvarCsv:
    ...
    def test_salvar_csv_encoding_utf8(self, tmp_path):
        df = pd.DataFrame(
            {"nome": ["José", "Mário"], "status": ["Concluído", "Em andamento"]}
        )
...
class TestCorrigirInconsistencia1:
    def test_corrigir_responsavel_simples(self):
        df_tempo = pd.DataFrame({"tarefa_id": [1, 2], "usuario": ["João", "Maria"]})
        df_tarefas = pd.DataFrame({"id": [1, 2], "responsavel": ["Pedro", "Ana"]})
    ...
    def test_sem_alteracoes_necessarias(self):
        df_tempo = pd.DataFrame({"tarefa_id": [1, 2], "usuario": ["Pedro", "Ana"]})
        df_tarefas = pd.DataFrame({"id": [1, 2], "responsavel": ["Pedro", "Ana"]})
    ...
    def test_tarefa_id_inexistente(self):
        df_tempo = pd.DataFrame({"tarefa_id": [1, 999], "usuario": ["João", "Maria"]})
...
class TestCorrigirInconsistencia2:
    def test_corrigir_status_projeto(self):
        ...
        df_programas = pd.DataFrame(
            {"id": [10, 11], "status": ["Concluído", "Em andamento"]}
        )
...
class TestCorrigirInconsistencia3:
    def test_corrigir_status_tarefa(self):
        ...
        df_projetos = pd.DataFrame(
            {"id": [100, 101], "status": ["Concluído", "Em andamento"]}
        )
...
class TestCorrigirInconsistencia4:
    def test_corrigir_tarefa_com_horas_passada(self):
        ...
        df_tempo = pd.DataFrame({"tarefa_id": [1], "horas_trabalhadas": [5.0]})
    ...
    def test_tarefa_sem_horas(self):
        df_tarefas = pd.DataFrame(
            {"id": [1], "status": ["Não iniciada"], "data_fim_prevista": ["2026-01-01"]}
        )
        df_tempo = pd.DataFrame({"tarefa_id": [1], "horas_trabalhadas": [0.0]})
    ...
    def test_tarefa_ja_iniciada(self):
        df_tarefas = pd.DataFrame(
            {"id": [1], "status": ["Em andamento"], "data_fim_prevista": ["2026-01-01"]}
        )
        df_tempo = pd.DataFrame({"tarefa_id": [1], "horas_trabalhadas": [5.0]})
    ...
    def test_data_fim_null(self):
        df_tarefas = pd.DataFrame(
            {"id": [1], "status": ["Não iniciada"], "data_fim_prevista": [None]}
        )
        df_tempo = pd.DataFrame({"tarefa_id": [1], "horas_trabalhadas": [5.0]})
...
class TestCorrigirProgramasConcluidos:
    def test_cascata_completa(self):
        df_programas = pd.DataFrame({"id": [1], "status": [STATUS_CONCLUIDO]})
        df_projetos = pd.DataFrame(
            {"id": [10], "programa_id": [1], "status": ["Em andamento"]}
        )
        df_tarefas = pd.DataFrame(
            {"id": [100], "projeto_id": [10], "status": ["Em andamento"]}
        )
    ...
    def test_multiplos_programas_e_projetos(self):
        df_programas = pd.DataFrame(
            {"id": [1, 2], "status": [STATUS_CONCLUIDO, "Em andamento"]}
        )
        df_projetos = pd.DataFrame(
            {
                "id": [10, 11],
...
class TestMain:
    @patch("builtins.print")
    @patch("api.management.commands.fix_csv.shutil.copy2")
    @patch("api.management.commands.fix_csv.salvar_csv")
    @patch("api.management.commands.fix_csv.corrigir_programas_concluidos")
    @patch("api.management.commands.fix_csv.corrigir_inconsistencia_4")
    @patch("api.management.commands.fix_csv.corrigir_inconsistencia_3")
    @patch("api.management.commands.fix_csv.corrigir_inconsistencia_2")
    @patch("api.management.commands.fix_csv.corrigir_inconsistencia_1")
    @patch("api.management.commands.fix_csv.criar_pasta_saida")
    @patch("api.management.commands.fix_csv.pd.read_csv")
    def test_main_sucesso(
        ...
        df_tempo = pd.DataFrame(
            {"tarefa_id": [1], "usuario": ["João"], "horas_trabalhadas": [8.0]}
        )
        df_tarefas = pd.DataFrame(
            {"id": [1], "responsavel": ["Pedro"], "projeto_id": [1]}
        )
        df_projetos = pd.DataFrame(
            {"id": [1], "programa_id": [1], "status": ["Em andamento"]}
        )
        df_programas = pd.DataFrame({"id": [1], "status": ["Em andamento"]})
    ...
    def test_main_com_dados_reais_simulados(
        ...
        df_programas = pd.DataFrame(
            {"id": [100, 101], "status": ["Em andamento", "Concluído"]}
        )
...
class TestIntegracaoCompleta:
    def test_fluxo_completo_correcoes(self):
        ...
        df_tarefas_corrigido = corrigir_inconsistencia_3(
            df_tarefas, df_projetos_corrigido
        )
...
class TestComFixture:
    def test_corr1_com_fixture(self, dados_teste):
        resultado = corrigir_inconsistencia_1(
            dados_teste["df_tempo"], dados_teste["df_tarefas"]
        )
        assert resultado.loc[0, "usuario"] == "Pedro"

    def test_corr2_com_fixture(self, dados_teste):
        resultado = corrigir_inconsistencia_2(
            dados_teste["df_projetos"], dados_teste["df_programas"]
        )
    ...
    def test_corr3_com_fixture(self, dados_teste):
        ...
        resultado = corrigir_inconsistencia_3(
            dados_teste["df_tarefas"], df_proj_corrigido
        )
