...
class TestCarregarFatoHoras:
    def _base(self):
        ...
        df_projetos = pd.DataFrame(
            {"id": [10], "programa_id": [100], "custo_hora": [100.0]}
        )
    ...
    def test_horas_zero_ignoradas(self):
        ...
        with patch("builtins.print"):
            carregar_fato_horas(
                df_tempo, df_tarefas, df_projetos, df_programas, df_func, cursor
            )
...
class TestCarregarFatoMateriais:
    def _base(self):
        ...
        df_solicitacoes = pd.DataFrame(
            {"id": [1, 2], "projeto_id": [10, 10], "material_id": [1, 2]}
        )
        df_pedidos = pd.DataFrame(
            {"solicitacao_id": [1, 2], "fornecedor_id": [100, 101]}
        )
        return (
            df_empenho,
            df_projetos,
...
class TestCarregarFatoCompras:
    def _base(self):
        ...
        df_cp = pd.DataFrame(
            {"pedido_compra_id": [10, 11], "valor_alocado": [1000.0, 2000.0]}
        )
    ...
    def test_lead_time_nulo_quando_sem_datas(self):
        ...
        with patch("builtins.print"):
            carregar_fato_compras(
                df_sol, df_ped, df_cp, df_proj, df_mat, df_forn, df_status, cursor
            )
        params = cursor.execute.call_args_list[1][0][1]
    ...
    def _dfs_vazios(self):
        return {
            ...
            "tempo_tarefas": pd.DataFrame(
                {"usuario": [], "data": [], "tarefa_id": [], "horas_trabalhadas": []}
            ),
            ...
            "compras_projeto": pd.DataFrame(
                {"pedido_compra_id": [], "valor_alocado": []}
            ),
            "estoque": pd.DataFrame(
                {"material_id": [], "projeto_id": [], "quantidade": []}
            ),
        }
...
class TestDevDbOrquestrador:
    ...
    def test_get_latest_migration_retorna_numero(self, mock_conn):
        ...
        result = (
            dev_db.get_latest_applied_migration.__wrapped__(mock_conn, "api")
...
class TestEdgeCases:
    ...
    def test_grande_quantidade_em_fato_materiais(self):
        ...
        with patch("builtins.print"):
            carregar_fato_materiais(
                df_empenho, df_proj, df_prog, df_mat, df_forn, df_sol, df_ped, cursor
            )
        assert cursor.execute.call_count == 2
