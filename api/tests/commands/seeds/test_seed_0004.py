...
class TestCarregarFatoHoras:
    def _make_dfs(self):
        df_projetos = pd.DataFrame({'id': [1], 'programa_id': [10], 'custo_hora': [50.0]})
    ...
    def test_inserts_record(self, mock_print):
        cursor = make_cursor()
        df_tempo, df_tarefas, df_projetos, df_programas, df_funcionario = self._make_dfs()
...
class TestCarregarFatoMateriais:
    def _make_dfs(self):
        ...
        df_solicitacoes = pd.DataFrame({'id': [1], 'projeto_id': [1], 'material_id': [1]})
    ...
    def test_fornecedor_id_none_when_no_match(self, mock_print):
        ...
        carregar_fato_materiais(df_empenho, df_projetos, df_programas, df_materiais,
                                df_fornecedores, df_solicitacoes, df_pedidos, cursor)
        insert_args = cursor.execute.call_args_list[1][0][1]
    ...
    def test_nan_fornecedor_excluded(self, mock_print):
        ...
        df_fornecedores = pd.DataFrame({'id': [1]})
        df_solicitacoes = pd.DataFrame({'id': [1], 'projeto_id': [1], 'material_id': [1]})
        df_pedidos = pd.DataFrame({'solicitacao_id': [1], 'fornecedor_id': [float('nan')]})
        df_empenho = pd.DataFrame({
            'projeto_id': [1],
            'material_id': [1],
            'quantidade_empenhada': [5],
...
class TestCarregarFatoCompras:
    def _make_dfs(self, status='Entregue', with_dates=True, with_valor_alocado=True):
        ...
        return df_solicitacoes, df_pedidos, df_compras_projeto, df_projetos, df_materiais, df_fornecedores, df_status_pedido
    ...
    def test_inserts_not_entregue(self, mock_print):
        ...
        df_solicitacoes = pd.DataFrame({
            ...
            'data_solicitacao': ['2023-03-01'], 'quantidade': [10],
            'status': ['Aberto'],
        })
        df_pedidos = pd.DataFrame({
            'solicitacao_id': [1], 'id': [1], 'fornecedor_id': [1],
            'data_pedido': ['2023-03-02'],
            'data_previsao_entrega': ['2023-03-10'],
            'status': ['Aberto'], 'valor_total': [100.0],
        })
        df_compras_projeto = pd.DataFrame({'pedido_compra_id': [1], 'valor_alocado': [100.0]})
        df_projetos = pd.DataFrame({'id': [1], 'programa_id': [10]})
        df_materiais = pd.DataFrame({'id': [1]})
        df_fornecedores = pd.DataFrame({'id': [1]})
        carregar_fato_compras(df_solicitacoes, df_pedidos, df_compras_projeto,
    ...
    def test_unknown_status_defaults_to_1(self, mock_print):
        ...
        df_solicitacoes = pd.DataFrame({
            'id': [1], 'projeto_id': [1], 'material_id': [1],
            'data_solicitacao': ['2023-03-01'], 'quantidade': [5],
            'status': ['Aberto'],
        })
        df_pedidos = pd.DataFrame({
            'solicitacao_id': [1], 'id': [1], 'fornecedor_id': [1],
            'data_pedido': [None], 'data_previsao_entrega': [None],
            'status': ['StatusDesconhecido'], 'valor_total': [200.0],
        })
        df_compras_projeto = pd.DataFrame({'pedido_compra_id': [1], 'valor_alocado': [200.0]})
        df_projetos = pd.DataFrame({'id': [1], 'programa_id': [10]})
        df_materiais = pd.DataFrame({'id': [1]})
        df_fornecedores = pd.DataFrame({'id': [1]})
        carregar_fato_compras(df_solicitacoes, df_pedidos, df_compras_projeto,
