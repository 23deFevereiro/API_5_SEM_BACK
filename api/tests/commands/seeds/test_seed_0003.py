...
class TestRun:
    ...
    def _patch_read_csv(self):
        df_programas = pd.DataFrame({
            ...
            'gerente_programa': ['G'], 'gerente_tecnico': ['T'],
            'data_inicio': ['2022-01-01'], 'data_fim_prevista': ['2023-12-31'],
            'status': ['Ativo'],
        })
        df_projetos = pd.DataFrame({
            'id': [1], 'codigo_projeto': ['PR001'], 'nome_projeto': ['Proj'],
            'programa_id': [1], 'responsavel': ['R'], 'custo_hora': [50.0],
            'data_inicio': ['2022-01-01'], 'data_fim_prevista': ['2023-12-31'],
            'status': ['Ativo'],
        })
        df_tarefas = pd.DataFrame({
            'id': [1], 'codigo_tarefa': ['T001'], 'projeto_id': [1],
            'titulo': ['Tar'], 'responsavel': ['R'], 'estimativa_horas': [8.0],
            'data_inicio': ['2022-01-01'], 'data_fim_prevista': ['2022-06-30'],
            'status': ['Aberta'],
        })
        df_materiais = pd.DataFrame({
            'id': [1], 'codigo_material': ['M001'], 'descricao': ['Mat'],
            'categoria': ['C'], 'fabricante': ['F'], 'custo_estimado': [10.0],
            'status': ['Ativo'],
        })
        ...
        df_solicitacoes = pd.DataFrame({
            'id': [1], 'numero_solicitacao': ['SC001'],
            'projeto_id': [1], 'material_id': [1], 'quantidade': [2],
            'data_solicitacao': ['2023-06-01'], 'prioridade': ['Alta'],
            'status': ['Aberto'],
        })
        df_pedidos = pd.DataFrame({
            'id': [1], 'numero_pedido': ['PC001'], 'fornecedor_id': [1],
            'data_pedido': ['2023-06-01'], 'data_previsao_entrega': ['2023-06-15'],
            'valor_total': [100.0], 'status': ['Entregue'],
        })
        df_compras_projeto = pd.DataFrame({
            'id': [1], 'pedido_compra_id': [1],
            'projeto_id': [1], 'valor_alocado': [100.0],
        })
        df_estoque = pd.DataFrame({
            'projeto_id': [1], 'material_id': [1],
            'quantidade': [5], 'localizacao': ['Dep A'],
        })
        return [
    ...
    def test_run_success(self, mock_print):
        ...
        with patch('seed_0003.get_connection', return_value=conn), \
