...
class TestListarProjetos:
    ...
    # TC-P01 — Cenário: Nenhum projeto cadastrado (camada de serviço)
    def test_retorna_lista_vazia_quando_nao_ha_projetos(self):
        resultado = listar_projetos()
        assert resultado == []

    # TC-P01 — Cenário: Retornar todos os projetos sem filtro (camada de serviço)
    def test_retorna_projetos_quando_existem(self):
        baker.make("api.DimProjeto", nome_projeto="Conversor DC-DC", _quantity=3)
        resultado = listar_projetos()
        assert len(resultado) == 3

    # TC-P01 — Cenário: Filtrar projetos por texto via search (camada de serviço)
    ...
    # TC-P01 — Contrato: cada item possui id, codigo_projeto e nome_projeto
    ...
    # TC-P01 — Cenário: Filtrar projetos por programa (camada de serviço)
    ...
    # Complementar ao TC-P01: programa_id ausente retorna todos os projetos
    ...
    # Complementar ao TC-P01: filtro por programa sem projetos retorna []
    ...
    # Complementar ao TC-P01: combinação dos filtros search + programa_id
...
class TestGetResumoProjeto:
    ...
    # TC-P03 — Cenário: Retornar resumo de projeto existente (cálculo de tempo_total)
...
class TestGetOverviewDataAll:
    ...
    # TC-P02 — Cenário: Nenhum dado de custo registrado (camada de serviço)
    ...
    # Complementar ao TC-P02: projetos concluídos entram no overview
    ...
    # TC-P02 — Cenário: Retornar série temporal de custos (camada de serviço)
    ...
    # TC-P02 — Contrato: cada item possui date_str e values
    ...
    # TC-P02 — Contrato: values contém codigo_projeto, nome_projeto e cost
    ...
    # TC-P02 — Cenário: Filtrar por programa_id (camada de serviço)
    def test_filtra_overview_por_programa_id(self):
        programa_a = baker.make("api.DimPrograma")
        programa_b = baker.make("api.DimPrograma")
        projeto_a = baker.make(
            "api.DimProjeto",
            nome_projeto="Do A",
            status="Em andamento",
            programa=programa_a,
        )
        projeto_b = baker.make(
            "api.DimProjeto",
            nome_projeto="Do B",
            status="Em andamento",
            programa=programa_b,
    ...
    # Complementar ao TC-P02: programa_id ausente retorna todos os projetos
    ...
    # Complementar ao TC-P02: programa sem projetos retorna []
...
class TestGetMateriaisProjeto:
    ...
    # Complementar ao TC-P04: projeto sem materiais retorna results=[] e count=0
    ...
    # TC-P04 — Cenário: Retornar primeira página de materiais (camada de serviço)
    def test_retorna_materiais_do_projeto(self):
        programa = baker.make("api.DimPrograma")
        projeto = baker.make("api.DimProjeto", programa=programa)
        material = baker.make(
            "api.DimMaterial", descricao="Capacitor", custo_estimado=10.00
        )
        fornecedor = baker.make("api.DimFornecedor")
        tempo = baker.make("api.DimTempo")
        baker.make(
    ...
    # TC-P04 — Contrato: custo_total_estimado calculado por material
    def test_calcula_custo_total_corretamente(self):
        programa = baker.make("api.DimPrograma")
        projeto = baker.make("api.DimProjeto", programa=programa)
        material = baker.make(
            "api.DimMaterial", descricao="Resistor", custo_estimado=20.00
        )
        fornecedor = baker.make("api.DimFornecedor")
        tempo = baker.make("api.DimTempo")
        baker.make(
    ...
    # Complementar ao TC-P04: empenhos do mesmo material são agregados
    ...
    def test_paginacao_retorna_page_size_correto(self):
        ...
        for i in range(15):
            material = baker.make(
                "api.DimMaterial", descricao=f"Material {i}", custo_estimado=10.00
            )
            baker.make(
                "api.FatoMateriais",
                projeto=projeto,
    ...
    # TC-P04 — Cenário: Navegar para segunda página (5 materiais restantes)
    def test_paginacao_segunda_pagina(self):
        ...
        for i in range(15):
            material = baker.make(
                "api.DimMaterial", descricao=f"Material {i:02d}", custo_estimado=10.00
            )
            baker.make(
                "api.FatoMateriais",
                projeto=projeto,
    ...
    # Complementar ao TC-P04: materiais de outros projetos não vazam
    def test_nao_retorna_materiais_de_outro_projeto(self):
        ...
        material = baker.make(
            "api.DimMaterial", descricao="Transistor", custo_estimado=15.00
        )
        fornecedor = baker.make("api.DimFornecedor")
        tempo = baker.make("api.DimTempo")
        baker.make(
    ...
    # TC-P04 — Cenário: Filtrar por período válido (data_inicio/data_fim)
    def test_filtra_materiais_por_periodo(self):
        programa = baker.make("api.DimPrograma")
        projeto = baker.make("api.DimProjeto", programa=programa)
        material = baker.make(
            "api.DimMaterial", descricao="Capacitor", custo_estimado=5.0
        )
        fornecedor = baker.make("api.DimFornecedor")
        ...
        resultado = get_materiais_projeto(
            projeto.id, data_inicio="2025-06-01", data_fim="2025-06-30"
        )
        assert resultado["count"] == 1
        assert resultado["results"][0]["quantidade"] == 10

    # Complementar ao TC-P04: filtro por nome de material com correspondência
    def test_filtra_materiais_por_nome(self):
        programa = baker.make("api.DimPrograma")
        projeto = baker.make("api.DimProjeto", programa=programa)
        capacitor = baker.make(
            "api.DimMaterial", descricao="Capacitor", custo_estimado=5.0
        )
        resistor = baker.make(
            "api.DimMaterial", descricao="Resistor", custo_estimado=3.0
        )
        fornecedor = baker.make("api.DimFornecedor")
        tempo = baker.make("api.DimTempo")
        baker.make(
        ...
        baker.make(
            ...
            tempo=tempo,
            quantidade_empenhada=1,
            custo_materiais=3.00,
        )
        resultado = get_materiais_projeto(projeto.id, material="cap")
        assert resultado["count"] == 1
        assert resultado["results"][0]["nome_material"] == "Capacitor"

    # TC-P04 — Cenário: Filtrar por nome de material inexistente (results=[] e count=0)
    def test_filtro_por_material_inexistente_retorna_vazio(self):
        programa = baker.make("api.DimPrograma")
        projeto = baker.make("api.DimProjeto", programa=programa)
        material = baker.make(
            "api.DimMaterial", descricao="Capacitor", custo_estimado=5.0
        ...
        resultado = get_materiais_projeto(projeto.id, material="materialinexistente")
        assert resultado["count"] == 0
        assert resultado["results"] == []
...
class TestGetMateriaisDisponiveis:
    ...
    def test_retorna_materiais_empenhados_distintos(self):
        ...
        projeto = baker.make("api.DimProjeto", programa=programa)
        capacitor = baker.make(
            "api.DimMaterial", descricao="Capacitor", custo_estimado=1.0
        )
        resistor = baker.make(
            "api.DimMaterial", descricao="Resistor", custo_estimado=1.0
        )
        fornecedor = baker.make("api.DimFornecedor")
        tempo = baker.make("api.DimTempo")
        baker.make(
...
class TestListarProjetosView:
    ...
    # TC-P01 — Status 200 na view de listagem de projetos
    ...
    # TC-P01 — Cenário: Filtrar projetos por programa (via query string na view)
...
class TestGetOverviewProjetosView:
    ...
    # TC-P02 — Status 200 na view de overview
    def test_retorna_200_para_get(self):
        factory = RequestFactory()
    ...
    # TC-P02 — Cenário: Filtrar por programa_id (via view)
    def test_view_filtra_overview_por_programa_id(self):
        ...
        projeto_do_programa = baker.make(
            "api.DimProjeto",
            nome_projeto="Do programa",
            status="Em andamento",
            programa=programa,
        )
        projeto_de_outro = baker.make(
            "api.DimProjeto",
            nome_projeto="De outro",
            status="Em andamento",
    ...
    # Complementar ao TC-P02: programa_id inválido é ignorado
...
class TestGetResumoProjetoView:
    ...
    # TC-P03 — Cenário: Projeto inexistente (404)
