...
class TestSugestaoProximaCompraSistema:
    ...
    @patch('api.views.compras_view.get_sugestao_proxima_compra')
    def test_retorna_200_para_get(self, mock_service, api_client):
        ...
        response = api_client.get('/api/compras/sugestao-proxima-compra/')

        assert response.status_code == 200
        assert response.json()['materiais'] == []

    def test_retorna_400_para_data_referencia_invalida(self, api_client):
        response = api_client.get(
            '/api/compras/sugestao-proxima-compra/',
            {'data_referencia': '10-01-2024'},
        )

        assert response.status_code == 400

    @patch('api.views.compras_view.get_sugestao_proxima_compra')
    def test_data_referencia_valida_chama_service(self, mock_service, api_client):
        mock_service.return_value = {
            'data_sugerida': '2024-01-10',
