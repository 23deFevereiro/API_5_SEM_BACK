...
class TestGetDistribuicaoStatus:
    ...
    def test_nao_inclui_projetos_de_outro_programa(self):
        ...
        for i in range(1, 6):
            baker.make(
                "api.DimProjeto", id=i, programa=programa2, status="Planejamento"
            )
...
class TestGetBurnupHorasProgramas:
    ...
    def test_separa_series_por_programa(self):
        ...
        programa2 = baker.make(
            "api.DimPrograma", codigo_programa="PROG-2", nome_programa="Beta"
        )
    ...
    def test_inclui_programas_independente_do_status_do_projeto(self):
        ...
        projeto = baker.make(
            "api.DimProjeto", id=1, programa=programa, status="Concluído"
        )
        tempo = self._make_tempo(2025, 1, 10)
        baker.make(
            "api.FatoHoras",
    ...
    def test_acumula_independente_para_cada_programa(self):
        ...
        valores_marco = {
            v["codigo_programa"]: v["horas"] for v in grupos["03/2025"]["values"]
        }
...
class TestGetBurnupCustoProgramas:
    ...
    def test_acumula_independente_para_cada_programa(self):
        ...
        grupos = {g["date_str"]: g for g in resultado}
        valores_marco = {
            v["codigo_programa"]: v["custo"] for v in grupos["03/2025"]["values"]
        }
...
class TestGetTabelaProjetos:
    ...
    def test_acao_corrigir_status_quando_todas_concluidas_e_em_andamento(self):
        programa = baker.make("api.DimPrograma")
        projeto = baker.make(
            "api.DimProjeto", id=1, programa=programa, status="Em andamento"
        )
        baker.make(
            "api.DimTarefa",
            id=1,
    ...
    def test_acao_corrigir_status_quando_todas_concluidas_e_planejamento(self):
        programa = baker.make("api.DimPrograma")
        projeto = baker.make(
            "api.DimProjeto", id=1, programa=programa, status="Planejamento"
        )
        baker.make(
            "api.DimTarefa",
            id=1,
    ...
    def test_acao_check_verde_quando_concluido_todas_tarefas_dentro_do_prazo(self):
        ...
        projeto = baker.make(
            ...
            programa=programa,
            status="Concluído",
            data_fim_prevista=date(2024, 6, 30),
        )
        tarefa = baker.make(
            "api.DimTarefa",
            id=1,
    ...
    def test_acao_check_vermelho_quando_concluido_todas_tarefas_fora_do_prazo(self):
        ...
        projeto = baker.make(
            ...
            programa=programa,
            status="Concluído",
            data_fim_prevista=date(2024, 5, 31),
        )
        tarefa = baker.make(
            "api.DimTarefa",
            id=1,
    ...
    def test_acao_check_amarelo_quando_concluido_tarefas_mistas(self):
        ...
        projeto = baker.make(
            ...
            programa=programa,
            status="Concluído",
            data_fim_prevista=date(2024, 6, 15),
        )
        tarefa1 = baker.make(
            "api.DimTarefa",
            id=1,
    ...
    def test_ordena_por_nome_projeto_asc(self):
        ...
        resultado = get_tabela_projetos(
            programa.id, sort_by="nome_projeto", sort_dir="asc"
        )
    ...
    def test_ordena_por_nome_projeto_desc(self):
        ...
        resultado = get_tabela_projetos(
            programa.id, sort_by="nome_projeto", sort_dir="desc"
        )
        nomes = [r["nome_projeto"] for r in resultado["results"]]
    ...
    def test_ordena_por_responsavel_desc(self):
        ...
        resultado = get_tabela_projetos(
            programa.id, sort_by="responsavel", sort_dir="desc"
        )
    ...
    def test_ordenacao_por_acao_pagina_corretamente(self):
        ...
        resultado = get_tabela_projetos(
            programa.id, page=2, sort_by="acao", sort_dir="asc"
        )
        assert resultado["count"] == 12
        assert resultado["page"] == 2
        assert len(resultado["results"]) == 2
