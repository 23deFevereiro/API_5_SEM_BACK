...
def carregar_fato_horas(
    df_tempo, df_tarefas, df_projetos, _df_programas, df_funcionario, cursor
):
...
def carregar_fato_materiais(
    ...
    for (proj, mat), grupo in df_sol_ped.groupby(["projeto_id", "material_id"]):
        fornecedor_map[(int(proj), int(mat))] = int(
            grupo["fornecedor_id"].mode().iloc[0]
        )
...
def carregar_fato_compras(
    ...
    status_map = {
        row["nome_status"]: row["id"] for _, row in df_status_pedido.iterrows()
    }
    ...
    for _, row in df_compras.iterrows():
        ...
        lead_time = None
        if pd.notna(row.get("data_previsao_entrega")) and pd.notna(
            row.get("data_pedido")
        ):
        ...
        quantidade_entregue = (
            quantidade_solicitada if row["status_ped"] == "Entregue" else 0
        )
        valor_alocado = (
            float(row["valor_alocado"]) if pd.notna(row.get("valor_alocado")) else 0.0
        )
...
def run(carregar_projeto_fn=None):
    ...
    try:
        ...
        df_status_pedido = pd.DataFrame(
            cursor.fetchall(), columns=["id", "nome_status"]
        )
