...
def corrigir_inconsistencia_2(df_projetos, df_programas):
    ...
    programas_concluidos = df_programas[df_programas["status"] == STATUS_CONCLUIDO][
        "id"
    ].tolist()
...
def corrigir_inconsistencia_3(df_tarefas, df_projetos):
    ...
    projetos_concluidos = df_projetos[df_projetos["status"] == STATUS_CONCLUIDO][
        "id"
    ].tolist()
...
def corrigir_inconsistencia_4(df_tarefas, df_tempo):
    ...
    horas_por_tarefa = (
        df_tempo.groupby("tarefa_id")["horas_trabalhadas"].sum().reset_index()
    )
    horas_por_tarefa.columns = ["tarefa_id", "total_horas"]

    tarefas_com_horas = set(
        horas_por_tarefa[horas_por_tarefa["total_horas"] > 0]["tarefa_id"].tolist()
    )
...
def corrigir_programas_concluidos(df_programas, df_projetos, df_tarefas):
    ...
    for idx, row in df_projetos.iterrows():
        if (
            row["programa_id"] in programas_concluidos
            and row["status"] != STATUS_CONCLUIDO
        ):
    ...
    for idx, row in df_tarefas.iterrows():
        if (
            row["projeto_id"] in projetos_concluidos
            and row["status"] != STATUS_CONCLUIDA
        ):
...
def main():
    ...
    df_projetos, df_tarefas = corrigir_programas_concluidos(
...
class Command(BaseCommand):
    help = (
        "Corrige inconsistências nos CSVs de origem e "
        "salva na pasta corrected_documents."
    )
