...
MIGRATION_WARNINGS = {
    "0003": (
        ...
        "    pedido_compra, "
        "solicitacao_compra, "
        "    empenho_material, estoque_material_projeto\n\n"
...
def ensure_corrected_documents():
    if not CORRECTED_DOCUMENTS_DIR.exists() or not any(
        CORRECTED_DOCUMENTS_DIR.glob("*.csv")
    ):
        print(
            "⚠️  Arquivos de corrected_documents não encontrados. "
            "Executando fix_csv..."
        )
...
def get_latest_applied_migration(app_label="api"):
    with connection.cursor() as cursor:
        ...
        if not row:
            raise CommandError(
                f"Nenhuma migration encontrada para o app '{app_label}'."
            )
...
def load_seed(migration_number):
    ...
    try:
        ...
        raise CommandError(
            ...
            f"Crie o arquivo seed_{migration_number}.py "
            "em api/management/commands/seeds."
        )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            help=(
                "Força a execução de um seed específico pelo número da "
                "migration (ex: 0004)."
            ),
        )
