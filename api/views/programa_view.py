...
TABELA_PROJETO_ITEM_SCHEMA = schema_obj(
    {
        ...
        "data_ultima_atividade": openapi.Schema(
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna a lista de programas cadastrados com "
        "filtro textual opcional pelo parametro search.",
    ),
    responses={
        200: resposta_sucesso(
            "Lista de programas cadastrados.", schema_array(PROGRAMA_SCHEMA)
        ),
    },
)
@api_view(["GET"])
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna o resumo consolidado do programa informado pelo identificador na URL."
    ),
    responses={
        200: resposta_sucesso(
            "Resumo consolidado do programa.", RESUMO_PROGRAMA_SCHEMA
        ),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna a distribuicao de status dos "
        "projetos associados ao programa informado."
    ),
    responses={
        200: resposta_sucesso(
            "Distribuicao de status dos projetos do programa.",
            DISTRIBUICAO_STATUS_SCHEMA,
        ),
        500: resposta_erro(
            "Erro interno ao buscar distribuicao de status do programa."
        ),
    },
)
@api_view(["GET"])
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna os dados consolidados de burnup de horas "
        "considerando todos os programas."
    ),
    responses={
        200: resposta_sucesso(
            "Serie temporal acumulada de horas por programa.",
            schema_array(BURNUP_GRUPO_HORAS_SCHEMA),
        ),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna os dados consolidados de burnup de custo "
        "considerando todos os programas."
    ),
    responses={
        200: resposta_sucesso(
            "Serie temporal acumulada de custo por programa.",
            schema_array(BURNUP_GRUPO_CUSTO_SCHEMA),
        ),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna as horas consolidadas por projeto dentro do programa informado."
    ),
    responses={
        200: resposta_sucesso(
            "Horas realizadas por projeto do programa.",
            schema_array(HORAS_PROJETO_SCHEMA),
        ),
        404: resposta_erro("Programa nao encontrado."),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna a tabela paginada de projetos do programa "
        "com ordenacao pelos campos suportados pela API."
    ),
    responses={
        200: resposta_sucesso(
            "Tabela paginada dos projetos do programa.",
            schema_paginada(TABELA_PROJETO_ITEM_SCHEMA),
        ),
...
def get_tabela_projetos_view(request, programa_id):
    try:
        ...
        if sort_dir not in ("asc", "desc"):
            sort_dir = "asc"
        dados = get_tabela_projetos(
            programa_id, page=page, page_size=10, sort_by=sort_by, sort_dir=sort_dir
        )
