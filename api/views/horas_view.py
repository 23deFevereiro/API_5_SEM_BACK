...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna as horas apontadas por funcionario em um projeto, "
        "com filtros opcionais por periodo e nome do funcionario."
    ),
    responses={
        200: resposta_sucesso(
            "Horas consolidadas por funcionario do projeto.",
            schema_array(HORAS_FUNCIONARIO_SCHEMA),
        ),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna os dados consolidados de burnup de horas dos projetos, "
        "podendo filtrar os resultados pelo parametro programa_id.",
    ),
    responses={
        200: resposta_sucesso(
            "Serie temporal de burnup de horas por projeto.",
            schema_array(BURNUP_PROJETO_SCHEMA),
        ),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna os nomes dos funcionarios vinculados ao projeto informado.",
    ),
    responses={
        200: resposta_sucesso(
            "Lista de nomes dos funcionarios do projeto.",
            schema_array(openapi.Schema(type=openapi.TYPE_STRING)),
        ),
