...
from ..services.alertas_svc import (
    get_alertas_materiais,
    get_estoque_tabela,
    get_sugestao_proxima_compra,
)
from ..services.compras_svc import (
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna os materiais com informacoes consolidadas "
        "para acompanhamento do processo de compras."
    ),
    responses={
        200: resposta_sucesso(
            "Lista de materiais encontrados no modulo de compras.",
            schema_array(MATERIAL_COMPRA_SCHEMA),
        ),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna os dados de lead time de um material especifico "
        "a partir do parametro de consulta material_id."
    ),
    responses={
        200: resposta_sucesso(
            "Historico de lead time do material informado.",
            schema_array(LEAD_TIME_ITEM_SCHEMA),
        ),
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna os alertas de materiais classificados por criticidade com base "
        "nos limites informados nos parametros critico_max e atencao_max."
    ),
    responses={
        200: resposta_sucesso(
            "Alertas agrupados por criticidade.", ALERTAS_RESPONSE_SCHEMA
        ),
...
def get_alertas_view(request):
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"error": "critico_max e atencao_max devem ser inteiros"}, status=400
            )
...
@swagger_auto_schema(
    ...
    operation_description=(
        "Retorna a tabela paginada de estoque com filtros por material e "
        "opcoes de ordenacao e classificacao de criticidade."
    ),
    responses={
        200: resposta_sucesso(
            "Tabela paginada de estoque dos materiais.",
            schema_paginada(ESTOQUE_ITEM_SCHEMA),
        ),
