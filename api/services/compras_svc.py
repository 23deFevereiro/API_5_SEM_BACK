...
def listar_materiais_com_compras():
    ids_compras = set(
        FatoCompras.objects.values_list("material_id", flat=True).distinct()
    )
    ids_materiais = set(
        FatoMateriais.objects.values_list("material_id", flat=True).distinct()
    )
    ids = ids_compras | ids_materiais
    materiais = (
        DimMaterial.objects.filter(id__in=ids)
...
def get_lead_time_por_material(material_id):
    compras = FatoCompras.objects.filter(
        material_id=material_id, lead_time__isnull=False
