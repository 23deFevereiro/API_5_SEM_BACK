...
def get_alertas_materiais(critico_max: int = 30, atencao_max: int = 60):
    ...
    estoque_map = {e['material_id']: e['estoque_total'] or 0 for e in estoque_qs}
    ...
    pendente_map = {
        p['material_id']: max(p['qtd_pendente'] or 0, 0)
        for p in pendentes_qs
    }
    ...
    for lt in lead_times_qs:
        mid = lt['material_id']
        if mid not in lead_time_map or lt['lt_medio'] < lead_time_map[mid][0]:
            lead_time_map[mid] = (float(lt['lt_medio']), lt['fornecedor__razao_social'])
