...
def _get_consumo_map(dias_periodo: int) -> dict[int, float]:
    consumo_qs = FatoMateriais.objects.values("material_id").annotate(
        total_empenhado=Sum("quantidade_empenhada")
    )
    return {
...
def _get_estoque_map() -> dict[int, int]:
    latest_date_subq = (
        FatoEstoque.objects.filter(material_id=OuterRef("material_id"))
        .values("material_id")
        .annotate(max_date=Max("tempo__data"))
        .values("max_date")[:1]
    )
    estoque_qs = (
        FatoEstoque.objects.annotate(latest_date=Subquery(latest_date_subq))
...
def _get_pendente_map() -> dict[int, int]:
    pendentes_qs = (
        FatoCompras.objects.filter(status__nome_status__in=PENDENTE_STATUS)
        .values("material_id")
        .annotate(
            qtd_pendente=Sum(
                ExpressionWrapper(
...
def get_alertas_materiais(critico_max: int = 30, atencao_max: int = 60):
    ...
    lead_time_map = _build_lead_time_map(
        FatoCompras.objects.filter(lead_time__isnull=False)
    ...
    nome_map = {
        m["id"]: m["descricao"]
        for m in DimMaterial.objects.filter(id__in=consumo_map.keys()).values(
            "id", "descricao"
        )
    }
...
def get_estoque_tabela(
    ...
    projeto_qs = (
        FatoMateriais.objects.filter(material_id__in=consumo_map.keys())
