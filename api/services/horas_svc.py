...
def get_horas_por_funcionario(
    projeto_id, data_inicio=None, data_fim=None, funcionario=None
):
    ...
    registros = (
        registros_qs.values("funcionario__nome")
...
def get_nomes_funcionarios_projeto(projeto_id):
    return sorted(
        FatoHoras.objects.filter(projeto_id=projeto_id)
...
def get_burnup_horas_projetos(programa_id=None):
    ...
    registros = (
        FatoHoras.objects.filter(**filtros)
        .values(
            "projeto__id",
